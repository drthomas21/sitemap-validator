...
//Allow unsigned SSL - use with extreme care
...
exports.CheckSitemap = function(url, code, source, destination, callback){
  request.get(url, function(error, response, body){
    if(error) {
      //throw err;
      throw error; //fixing undefined variable
    }
...
function validateUrl(task, callback){
  ...
  request.get({url: replacedUrl, followRedirect: false, timeout: 120000}, function(error, resp, body){
    if(error) {
      console.log('Bad URL, ' + task.url);
      callback();
      return;
